...
#   runtime.tar.gz   – compressed dist/ directory (contains media/ subdir)
#   checksums.txt    – sha256 checksum for the tarball
#
# Note: Vite outputs to dist/media/ (base: '/media/'). We tar from dist/ so the
# tarball preserves the media/ prefix and extracts into hosting/public/media/.
set -euo pipefail
...
if [[ ! -d "$RUNTIME_SRC/media" ]]; then
  echo "ERROR: Runtime build output not found at $RUNTIME_SRC/media — run 'npm run build' first."
  exit 1
fi
